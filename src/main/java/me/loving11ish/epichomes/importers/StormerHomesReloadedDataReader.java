    ...
    public StormerHomesReloadedDataReader() {
        try {
            ...
            if (config.getBoolean("general.developer-debug-mode.enabled")||!usermapConfig.getBoolean("import-completed")){
                logger.info(ColorUtils.translateColorCodes("&6EpicHomes-Debug: &aFound StormerHomesReloaded config.yml and loaded successfully."));
            }
            if (loadHomes()){
                this.homesImportSuccessful = true;
            }
        }catch (IOException | InvalidConfigurationException | NullPointerException e) {
            logger.severe(ColorUtils.translateColorCodes("&6EpicHomes: &4&lCannot find StormerHomesReloaded data file!"));
        }
        this.world = null;
        this.homeName = null;
    ...
    private boolean loadHomes() {
        try {
            if (fileReader.getConfigurationSection("homes2") != null){
                if (config.getBoolean("general.developer-debug-mode.enabled")||!usermapConfig.getBoolean("import-completed")){
                    logger.info(ColorUtils.translateColorCodes("&6EpicHomes-Debug: &aFound homes2 section in StormerHomesReloaded config.yml"));
                }
                fileReader.getConfigurationSection("homes2").getKeys(false).forEach(key -> {
                    ...
                    fileReader.getConfigurationSection("homes2." + key).getKeys(false).forEach(home -> {
                        homeName = home;
                        if (config.getBoolean("general.developer-debug-mode.enabled")||!usermapConfig.getBoolean("import-completed")){
                            logger.info(ColorUtils.translateColorCodes("&6EpicHomes-Debug: &aFound home name entry of: " + home + " &afor UUID: " + key));
                        }
                        world = fileReader.getString("homes2." + key + "." + home + "." + "world");
                        if (config.getBoolean("general.developer-debug-mode.enabled")||!usermapConfig.getBoolean("import-completed")){
                            logger.info(ColorUtils.translateColorCodes("&6EpicHomes-Debug: &aFound world entry for home: " + home + ". &aWorld entry: " + world));
                        }
                        playerName = fileReader.getString("homes2." + key + "." + home + "." + "playername");
                        if (config.getBoolean("general.developer-debug-mode.enabled")||!usermapConfig.getBoolean("import-completed")){
                            logger.info(ColorUtils.translateColorCodes("&6EpicHomes-Debug: &aFound playername entry for home: " + home + ". &aPlayerName entry: " + playerName));
                        }
                        x = fileReader.getDouble("homes2." + key + "." + home + "." + "x");
                        if (config.getBoolean("general.developer-debug-mode.enabled")||!usermapConfig.getBoolean("import-completed")){
                            logger.info(ColorUtils.translateColorCodes("&6EpicHomes-Debug: &aFound x entry for home: " + home + ". &aX entry: " + x));
                        }
                        y = fileReader.getDouble("homes2." + key + "." + home + "." + "y");
                        if (config.getBoolean("general.developer-debug-mode.enabled")||!usermapConfig.getBoolean("import-completed")){
                            logger.info(ColorUtils.translateColorCodes("&6EpicHomes-Debug: &aFound y entry for home: " + home + ". &aY entry: " + y));
                        }
                        z = fileReader.getDouble("homes2." + key + "." + home + "." + "z");
                        if (config.getBoolean("general.developer-debug-mode.enabled")||!usermapConfig.getBoolean("import-completed")){
                            logger.info(ColorUtils.translateColorCodes("&6EpicHomes-Debug: &aFound z entry for home: " + home + ". &aZ entry: " + z));
                        }
                        yaw = (float) fileReader.getDouble("homes2." + key + "." + home + "." + "yaw");
                        if (config.getBoolean("general.developer-debug-mode.enabled")||!usermapConfig.getBoolean("import-completed")){
                            logger.info(ColorUtils.translateColorCodes("&6EpicHomes-Debug: &aFound yaw entry for home: " + home + ". &aYaw entry: " + yaw));
                        }
                        pitch = (float) fileReader.getDouble("homes2." + key + "." + home + "." + "pitch");
                        if (config.getBoolean("general.developer-debug-mode.enabled")||!usermapConfig.getBoolean("import-completed")){
                            logger.info(ColorUtils.translateColorCodes("&6EpicHomes-Debug: &aFound pitch entry for home: " + home + ". &aPitch entry: " + pitch));
                        }
                        World homeWorld = Bukkit.getWorld(world);
                        Location location = new Location(homeWorld, x, y, z, yaw, pitch);
                        homes.put(homeName, location);
                        if (config.getBoolean("general.developer-debug-mode.enabled")||!usermapConfig.getBoolean("import-completed")){
                            logger.info(ColorUtils.translateColorCodes("&6EpicHomes-Debug: &aHome: " + home + " &ahas been successfully created for player: " + playerName));
                        }
                    });
                        ...
                        for (Map.Entry<String, Location> homesList : homes.entrySet()){
                            if (!usersCurrentHomes.containsKey(homesList.getKey())){
                                ...
                                if (config.getBoolean("general.developer-debug-mode.enabled")||!usermapConfig.getBoolean("import-completed")){
                                    ...
                                    logger.info(ColorUtils.translateColorCodes("&6EpicHomes-Debug: &aSuccessfully added new home: " + homeName + " to player."));
                                }
                            }else {
                                if (config.getBoolean("general.developer-debug-mode.enabled") || !usermapConfig.getBoolean("import-completed")) {
                                    logger.info(ColorUtils.translateColorCodes("&6EpicHomes-Debug: &aPlayer " + user.getLastKnownName() + " already exists in the usermap"));
                                    logger.info(ColorUtils.translateColorCodes("&6EpicHomes-Debug: &aThe current home iteration already exists! Skipping..."));
                                }
                            }
                        }
            ...
            logger.severe(ColorUtils.translateColorCodes("&6EpicHomes &cCould not load homes from StormerHomesReloaded plugin!"));
